...
format="$(python3 -c "import json;print(json.load(open('config.json'))[0]['format']['current'])")"
...
if [ -n "$abi" ] && [ "$format" != "$abi" ]; then
  echo "config.json format.current ($format) != QUANTION_ABI_VERSION ($abi)"
  failed=1
fi
